 ...
 * Spatial Match
 *
 * Determine a password character proximity or distance on
 * a QWERTY or Dvorak keyboard layout.
 *
...
class SpatialMatch extends Match
{
    ...
    /**
     * Count of shifted (uppercase or symbol) characters.
     *
     * @var integer
     */
    public $shiftedCount;
    ...
    /**
     * Relates to possible turns required for pattern.
     *
     * @var integer
     */
    public $turns;
    ...
    /**
     * Keyboard layout being used for spatial match.
     *
     * @var string
     */
    public $graph;
    ...
    public function __construct($password, $begin, $end, $token, $params = array())
    {
        ...
        // Preset properties since adjacency graph is constant for
        // QWERTY keyboard and keypad.
    ...
    protected static function graphMatch($password, $graph)
    {
        ...
        while ($i < $passwordLength - 1) {
            ...
            while (true) {
                ...
                // Consider growing pattern by one character if $j hasn't
                // gone over the edge.
                if ($j < $passwordLength) {
                    $curChar = $password[$j];
                    ...
                    foreach ($adjacents as $adj ) {
                        ...
                        if ($adj && $curCharPos !== -1) {
                            ...
                            if ($curCharPos === 1) {
                                ...
                                // 0 means unshifted: A vs a, % vs 5, etc. for example, 'q'
                                // is adjacent to the entry '2@'. @ is shifted w/ index 1, 2 is unshifted.
                                $shiftedCount += 1;
                            }

                            if ($lastDirection !== $foundDirection) {
                                // adding a turn is correct even in the initial case when
                                // last_direction is null:  every spatial pattern starts with
                                // a turn.
                                $turns += 1;
